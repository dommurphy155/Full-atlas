#!/usr/bin/env python3
"""
AgentMail API wrapper for OpenRouter email verification.
Usage:
  python3 agentmail.py create  -> Nukes all existing inboxes, creates fresh one, prints email.
  python3 agentmail.py check   -> Polls the last created inbox for OpenRouter verification URL.
...
from config import ROOT_DIR, DATA_DIR, STATE_FILE, ENV_FILE, AGENTMAIL_API_KEY, AGENTMAIL_BASE
            ...
            while time.time() - start < timeout:
                try:
                    ...
                    for msg in msgs:
                        ...
                        # Look for any openrouter.ai link in the email
                        m = re.search(r'https://openrouter\.ai/[^\s"\'<>]+', body)
                        if m:
                            return m.group()

                        # Also check for relative links that might be in the email
                        m = re.search(r'(?:href|src)=["\']([^"\']*openrouter\.ai[^"\']*)["\']', body, re.I)
                        if m:
                            return m.group(1)
                except Exception:
                    pass
...
async def cmd_check():
    ...
    print(f"[AgentMail] Polling inbox {inbox_id} for OpenRouter verification URL...", file=sys.stderr)
